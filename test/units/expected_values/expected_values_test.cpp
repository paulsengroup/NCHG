...
TEST_CASE("ExpectedValues", "[long][expected_values]") {
  ...
  SECTION("genome-wide") {
    ...
    SECTION("expected matrix") {
      SECTION("cis") {
        const auto chrom = clr->chromosomes().at("chr22");
        const auto num_bins = (chrom.size() + clr->resolution() - 1) / clr->resolution();
        CHECK(evs.expected_matrix(chrom).num_rows() == num_bins);
      }
      SECTION("trans") {
        const auto chrom1 = clr->chromosomes().at("chr21");
        const auto chrom2 = clr->chromosomes().at("chr22");

        CHECK(evs.expected_matrix(chrom1, chrom2).nnz_avg() == evs.expected_value(chrom1, chrom2));
      }
    }
