...
def run_or_print_helper(
    ...
    run_or_print(base_args + ["--settings", f"compiler.cppstd={cppstd}"], env=env, dry_run=dry_run)
    run_or_print(
    ...
    run_or_print(
        base_args + ["--settings", f"compiler.cppstd={cppstd}", "--options", "NCHG/*:with_duckdb_only=True"],
        env=env,
        dry_run=dry_run,
    )
...
def run_conan(
    ...
    conan_args = add_output_params(conan_args, output_folder, deploy)
    ...
    run_or_print_helper(conan_args, cppstd, env, dry_run)
...
def run_ci_linux(args: Dict[str, Any]) -> str:
    ...
    run_or_print_helper(conan_args, cppstd)
